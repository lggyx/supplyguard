...
>
> **版本**：v2.3（2026-08-31）。v2.3：§8 工作流由六步循环升级为**七步循环**——新增第 4 步「运行时功能验证」（实现 → 测试 → 运行时验证 → 提交的强制闭环；含浏览器/真实调用与控制台门禁），§8 内部小节编号顺延，§10/§16 联动，RUBRICS.md 与 PROMPT_BOOTSTRAP.md 同步更新（维护者要求的收紧性变更）。v2.2（2026-08-30）：全条款细化（不变量合规做法、Web API 与设计令牌、分阶段单元清单、循环模板、迁移映射、反模式清单）。§1–§16 编号自 v2.1 起冻结——RUBRICS.md 与子 Prompt 依赖这些编号引用。
...
**永不裁剪**：七步循环（§8，含第 4 步运行时功能验证）、§5 全部不变量、测试全绿、**可运行优先原则**——任何时刻仓库必须处于可 build、可 test 状态；功能未完成也不允许破坏绿灯。
...
**每个功能单元必须完整走完以下七步，顺序固定，不得合并、不得跳步。** 一个循环 = 一个 git 提交。**静态测试全绿不代表完成——只有第 4 步运行时验证通过的功能单元才允许进入后续步骤直至提交。**
...
### 8.4 第 4 步：运行时功能验证（真实调用 + 控制台门禁，强制）

静态测试全绿 ≠ 功能可用。本步在真实运行态验证本单元的功能行为，是提交前最后的技术门禁：

- **运行验证**：CLI / 扫描类单元用真实命令 + 真实输入跑通（如 `cargo run -- scan <fixture 项目>`）；Web 控制台相关单元先 `cargo run -- serve` 启动，**用浏览器打开页面实际调用本单元功能**（会话具备浏览器操作能力；环境不支持浏览器时降级为按用户真实操作路径逐接口发 HTTP 请求，并在报告中显式声明降级）；
- **控制台门禁**：全程监控 stdout / stderr 与 tracing 日志——出现 ERROR、panic、未捕获 warning、异常堆栈即为不通过（§14 允许清单内的预期警告除外，须逐条注明依据）；浏览器场景同时核对页面无报错提示、渲染与交互结果符合本单元预期；
- **失败处理**：任一不符 → 回到第 1 步修复，修复后**重走第 2–4 步**（同时补上让该问题漏网的回归测试用例）；
- **证据留存**：在回复中贴关键证据——执行命令与输出摘要、浏览器验证的操作路径与结果、日志尾部无错摘录；冲刺交付报告 §3 必须汇总；
- **禁止**：以"测试过了就当功能没问题"跳过本步；以"预计能跑"代替实际运行；复用上一单元的运行证据冒充本单元证据。

### 8.5 第 5 步：逻辑验证（输出结论清单）
...
- Runtime: scan on fixture project pass; serve + browser invocation
  OK; console clean (no ERROR / unexpected warning)
- Refs: design doc S03; invariants §5.3/§5.7
```

### 8.8 单元粒度与禁止事项

- 一个 Skill / 一个模块 / 一个行为切片；单个循环 diff 以 ≤ ~400 行为宜；**时间压力下允许更小（甚至单函数级），但七步一项不许省**；
- 禁止跨单元批量推进后一次性提交；禁止红灯提交（唯一例外：revert）；禁止"先提交再补测试"；
- 提交后发现的缺陷：新开一个循环走完七步，以 `fix` 提交修复，禁止悄悄 amend 已推送历史；
- Definition of Done：七步全过（含第 4 步运行时功能验证）+ 文档同步（§11）。
...
- 提交正文模板见 §8.7；`Cargo.lock` 变更随对应功能提交；
- **每个阶段（S1–S6）结束推送一次 `main`；冲刺收尾必须推送**；禁止 force-push；
- 禁止提交：测试红灯、**运行时功能验证（§8.4）未执行或未通过的功能单元**、密钥与凭据、真实攻击载荷、（S1 之后）任何 Python / uv 残留、构建产物（`target/` 已被 .gitignore 覆盖，确认之）；
...
- **诚实交付**：每个循环结束时如实报告——改动范围、测试结果、运行时验证结果（§8.4）、验证与交叉检验结论、遗留问题；不报"大概通过了"。
...
运行时验证（§8.4）：逐单元列真实调用 / 浏览器验证结果与控制台结论（ERROR/警告清零证据）
