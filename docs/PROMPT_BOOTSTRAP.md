...
| D-6 | Git | 全部提交已推送 `main`；历史呈现单元级七步循环节奏 |
| D-7 | 报告 | 最终交付报告（§9.3 模板）与 git/运行证据交叉一致 |

### 1.4 你将被打分（与 RUBRICS.md 对应）

维度：可运行性与底线 25%、七步循环纪律 15%、不变量 15%、测试质量 15%、时间管理与裁剪 10%、Web 可视化 10%、报告与诚实性 10%；≥90 且无一票否决 = 优秀。**一票否决**（触发任一直接不合格）：伪造测试、红灯提交、审计可改写或 untrusted 原文外泄、提交真实密钥/载荷、引 Node/npm、砍底线、改三份约束文件放水、红灯仓库冒充完成。按本手册执行自然得分，不要表演、不要走捷径。
...
| 文件 | 角色 | 何时读 |
| --- | --- | --- |
| `docs/PROMPT.md` | 主 Prompt：§3 代码风格、§4 模块契约、§5 十条不变量、§6 范围与 Web 规格、§7 冲刺计划、§8 七步循环、§9 测试边界、§13 迁移映射、§14 默认值、§17 反模式 | 第 2 步完整精读；各阶段动工前复读对应章节 |
...
| 序 | 材料 | 时间 | 提取重点 |
| --- | --- | --- | --- |
| 1 | `docs/PROMPT.md` 完整精读 | ≤10 min | §3.3 代码红线；§4.2 模块"必须/禁止"表；§5 十条不变量（背下）；§7 阶段表+底线+裁剪阶梯；§8 七步循环与提交模板（重点背下 §8.4 运行时功能验证门禁）；§14 默认值；§17 反模式 |
...
0:25 / 2:25 / 3:20 对表，**落后 >10 分钟按主 Prompt §7.2 裁剪阶梯降档**，顺序固定：S5 收缩 → Web 降只读报告页 → license-check stub → hallucination 离线化。底线与永不裁剪项（七步循环、§5 不变量、全绿、可运行优先）不可触碰。裁剪是决策不是失败，报告如实写第几档。

### 5.3 可运行优先

任何时刻 `cargo build` 必须成功；开工前是绿的，做完还是绿的；做坏先修或回退该单元，红灯不过夜。

## 6. 七步循环操作细则（每单元一次循环一个提交，主 Prompt §8 为准）

单元 = 一个 Skill / 一个模块 / 一个行为切片（diff ≤ ~400 行，时间紧可更小，七步一项不省）。
...
| 4 运行时验证 | **强制门禁**：CLI 单元真实命令+真实输入跑通；Web 单元 `serve` 后**浏览器实际调用**；控制台/日志无 ERROR、panic、未捕获警告（§14 允许除外）；证据贴进回复；败则回步 1 并补回归用例 | 运行证据 |
| 5 逻辑验证 | 清单：输入输出✓、错误分支✓、边界✓、§5 逐条✓ | 验证清单 |
| 6 交叉检验 | 四视角：规格重推 / 测试审计（断言规格非实现）/ 对抗反例 / 契约一致性；矛盾回步 1 | 检验结论 |
| 7 git 提交 | `type(scope): summary` + 正文（范围/验证/运行时/遗留），对照主 Prompt §8.7 | 一个提交 |

### 6.1 完整示范（`license_check` 迷你 walkthrough）

推演：输入=包 license 数组+策略；输出=`violations + compatible + policy_version`；边界=空列表、未知 license、策略缺失；降级=未知 license → "需人工确认"不自动 block（§5.7）。测试（表驱动）：`allows_permitted` / `flags_denied` / `unknown_requires_review` / `empty_yields_compatible` / `missing_policy_defaults_strict`。交叉检验发现：`GPL-3.0` vs `gpl-3.0` 应规范化而非视为未知——补规范化与用例；对抗例：空字符串 license → 走未知路径 ✓。运行时验证：对含 GPL 依赖的 fixture 项目跑 `scan`，控制台无 ERROR，输出含"需人工确认" ✓。提交：`feat(skills): add license-check with conservative unknown handling`。照此粒度执行每个单元。
...
### S3（1:05–2:25）——主战场，顺序不可换（每单元一次七步循环）
